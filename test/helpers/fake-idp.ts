/**
 * In-process fake OpenID Provider for `test/sso.test.ts`.
 *
 * A typed port of `Vibe-Auth/packages/client/test/fake-idp.ts` (by way of
 * `trial-balance-app/test/fake-idp.mjs`). The published `@kisaesdevlab/vibe-auth` ships only
 * `dist/` and `sql/`, so the fake provider has to live in each product; keep its behaviour
 * identical to upstream when re-syncing. It serves discovery, JWKS, authorization (it
 * auto-consents as the configured user), token (PKCE S256 + client_secret_basic), userinfo
 * and end-session, and mints back-channel logout tokens.
 *
 * What matters most for this product is `user.amr`: it is the claim the mandatory second
 * factor rests on (QUESTIONS.md Q18), so the tests drive it directly.
 */
...
export interface FakeUser {
  ...
  groups?: string[];
  roles?: string[];
  /** Defaults to `['pwd']` — a password and nothing else, which this app must refuse. */
  amr?: string[];
}
...
export class FakeIdp {
  ...
  get issuer(): string {
    return `${this.base}/application/o/vibe-1040/`;
  }
